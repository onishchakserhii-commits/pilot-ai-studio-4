...
type Props = {
  postsByLang: Record<string, BlogPost[]>;
};

export function BlogListingClient({ postsByLang }: Props) {
  const { t, lang } = useTranslation();
  
  // Use the requested language's posts, or fallback to French if none exist
  let posts = postsByLang[lang] || [];
  if (posts.length === 0 && postsByLang['fr']) {
    posts = postsByLang['fr'];
  }
